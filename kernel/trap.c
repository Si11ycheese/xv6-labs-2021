...
void usertrap(void) {
  ...
  if (r_scause() == 13 || r_scause() == 15) // page fault
  {
    ...
    if (va >= p->sz || va < p->trapframe->sp) {
      // page-faults on a virtual memory address higher than any allocated with
      // sbrk() or lower than the stack. In xv6, heap is higher than stack
      p->killed = 1;
    } else {
      int i;
      ...
      for (i = 0; i < NVMA; i++) {
        if (p->vmas[i].valid) {
          if (p->vmas[i].addr <= va &&
              (p->vmas[i].addr + p->vmas[i].length) > va)
            break;
        }
      }
      if (i == NVMA) {
